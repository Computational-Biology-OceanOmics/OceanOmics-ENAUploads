...
for line in open('../1-samples/RESULTS.xml'):
    ...
    if '<SAMPLE' in line:
        ...
        oldalias = alias
        if alias == 'RS21FEB_NTC':
            alias = 'NTC'
        if alias.startswith('RS21FEB_EB'):
            alias = alias.replace('RS21FEB_','')
        ...
        if sixteen_f:
            ...
            print(f'Sixteen F not found for {alias}')
            print(f'Sixteen R not found for {alias}')
